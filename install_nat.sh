...
# --- 1. Ввод данных от пользователя с очисткой пробелов ---
echo "--- Настройка Iptables DNAT/MASQUERADE ---"
echo ""

# Запрос IP зарубежного сервера и очистка пробелов
read -p "Введите IP-адрес зарубежного сервера: " FOREIGN_IP_RAW
...
read -p "Введите номер порта, который нужно перенаправить: " PORT_RAW
...
# --- 2. Получение сетевого интерфейса (Остальная часть скрипта без изменений) ---
...
if [[ -z "$INTERFACE" ]]; then
    ...
    read -p "Введите внешний интерфейс вручную (например, eth0): " INTERFACE
    if [[ -z "$INTERFACE" ]]; then
...
echo "-----------------------------------"
read -p "Нажмите Enter для продолжения установки..."
...
echo "В вашей конфигурации AWG поменяйте IP адрес на $FOREIGN_IP"
